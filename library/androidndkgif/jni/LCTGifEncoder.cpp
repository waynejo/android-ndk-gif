...
void LCTGifEncoder::qsortColorHistogram(uint32_t* imageColorHistogram, int32_t maxColor, uint32_t from, uint32_t to)
{
	if (to == from) {
		return ;
	}
...
void LCTGifEncoder::updateColorHistogram(Cube* nextCube, Cube* maxCube, int32_t maxColor, uint32_t* imageColorHistogram)
{
...
void LCTGifEncoder::computeColorTable(uint32_t* pixels, Cube* cubes)
{
...
void LCTGifEncoder::reduceColor(Cube* cubes, uint32_t cubeNum, uint32_t* pixels)
{
